* 组件
	...
	* state属性是作为自己的数据 可以被修改	
		*  初始化 this.state={}
		*  更新会合并之前的 this.setState({},callback)
			*  	重新渲染后调用
			*  不会立即刷新渲染 会批量渲染
		* replaceState({},call) 替换state
		*  es6 this.state={}
...
* 组件生命周期
	...
	```
	getDefaultProps   继承Component(es6) 组件不会调用 es5会调用
...
* FlatList
	...
	```
		1:包含FlatList的组件 不要继承React.PureComponent
...
* SectionList
	...
	```
		...
		renderSectionHeader?: ?(info: {section: SectionT}) => ?React.Element<any>  组头组件
	...
	分组:
		所有Cell 都是一致的 分割线一致 cell-key 同一分配
			{
				renderItem={}
