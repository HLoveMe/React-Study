...
import { SettingGroup ,SettingModel,BaseModel} from "./SettingModel"
class IndexPath{
    section = 0;
    index = 0;
...
export class SettingView extends Component{
    ...
    _isCustomItem=(item)=>{
      return (item instanceof BaseModel) && !(item instanceof SettingModel)
    };

    _renderItem=({item,index})=>{
        //已经适配 自定义
        return (
            <item.component  item={ item } onPress={ this._didclick } index={ new IndexPath(0,index) }>
        ...
        for (var j = 0 ;j<this.state.items.length;j++){
            ...
            var one = {
                ...
                renderItem:({item,index})=>{
                    //已经适配 自定义
                    return (
                        <item.component  item={ item } onPress={ this._didclick } index={ new IndexPath(one.index,index) }>
