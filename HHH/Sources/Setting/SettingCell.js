...
class Cell extends  Component{
    ...
    _click = ()=>{
        if(this.props.onPress){
            this.props.onPress(this.state.item,this.props.index);
        }
    };
    _iconRender=()=>{
...
export class  CustomSettingCell extends Component{
    constructor(ops){
        super(ops);
        /** 可用的参数
         *  item 表示该cell 拥有的模型
         *  index   {section: index:}
         *  onPress  点击回调 onPress(item,index)
         *
         *
         * */
    }
    render(){
        ...
        return (
            <TouchableOpacity onPress={ ()=>{this.props.onPress(item,index)}}>
                <Text style={{height:30,lineHeight:30,textAlign:"center",backgroundColor:'orange'}}>
                    {
                        item.data.title
                    }
                </Text>
